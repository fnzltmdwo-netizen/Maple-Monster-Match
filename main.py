...
BLOCKED_JSON_PATH = "blocked_monsters.json"
...
def safe_user_name(name):
    return str(name).strip()[:12] or "친구"
...
def save_blocked_monsters(blocked_set):
    data = sorted(list(blocked_set))
...
def load_clip_engine():
    ...
    if not os.path.exists(CLIP_VECTOR_PATH):
        raise FileNotFoundError(
            "monster_clip_vectors.pkl 파일이 없습니다. 먼저 python clip_build_vectors.py 실행 후 GitHub에 업로드하세요."
        )
    ...
    if clip_model is None or clip_preprocess is None:
        ...
        clip_model.eval()
        print("CLIP model loaded:", CLIP_DEVICE)

    if clip_monsters is None:
        print("Loading monster vectors...")
        with open(CLIP_VECTOR_PATH, "rb") as f:
            clip_monsters = pickle.load(f)
        print("Monster vectors loaded:", len(clip_monsters))
...
def clip_score_to_percent(score, top_score=None, rank=0):
    if top_score and top_score > 0:
        relative = score / top_score
        base = int(72 + relative * 24)
    else:
        base = int((score + 1) / 2 * 100)

    if rank == 0:
        return max(90, min(base, 98))
    if rank == 1:
        return max(82, min(base, 91))
    return max(74, min(base, 86))
...
def run_clip_match(image_base64: str, top_k=3):
    _, _, monsters = load_clip_engine()
    blocked = load_blocked_monsters()

    image = base64_to_pil(image_base64)
    user_vec = clip_image_vector(image)
    ...
    for monster in monsters:
        ...
        scored.append({
            "name": name,
            "mob_id": str(monster.get("mob_id", "")).strip(),
            "image_url": str(monster.get("image_url", "")).strip(),
            "source_url": str(monster.get("source_url", "")).strip(),
            "score": round(score, 5),
            "reason": "",
            "common_tags": [],
            "tags": ["clip", "image-vector"],
            "species_tag": "",
        })
    ...
    for idx, item in enumerate(scored[:top_k]):
        copied = dict(item)
        copied["percent"] = clip_score_to_percent(copied["score"], top_score, idx)
...
def make_og_image(data):
    ...
    for i, monster in enumerate(results):
        ...
        monster_img = fetch_monster_image(monster.get("image_url", ""))
        if monster_img:
        ...
        draw_center_text(
            ...
            f"매칭도 {monster.get('percent', 90)}%",
            percent_font,
            "#6370ff",
        )

        reason = str(monster.get("reason", "전체 분위기가 비슷해요!"))[:45]
...
def admin_page(q: str = "", only_blocked: int = 0):
    try:
        _, _, monsters = load_clip_engine()
    except Exception as e:
        return HTMLResponse(f"<h1>CLIP 로딩 실패</h1><pre>{escape_html(str(e))}</pre>")
    ...
    for m in monsters:
        ...
        status = "제외됨" if is_blocked else "사용중"
        btn_text = "사용하기" if is_blocked else "제외하기"
        action = "unblock" if is_blocked else "block"
        cls = "blocked" if is_blocked else ""

        cards += f"""
        <div class="card {cls}">
          <img src="{escape_html(image_url)}" onerror="this.style.opacity=.2" />
...
<head>
  ...
  <style>
    ...
    .card {{
      ...
      padding: 14px;
      text-align: center;
      box-shadow: 0 8px 24px rgba(74,86,160,.08);
    }}
    .card.blocked {{
      opacity: .45;
      border-color: #ff8a8a;
...
<body>
  <div class="top">
    <h1>몬스터 관리자</h1>
    <p>제외된 몬스터: {len(blocked)}개 / 전체 후보: {len(monsters)}개</p>

    <form class="search" method="get" action="/admin">
      <input name="q" value="{escape_html(q)}" placeholder="몬스터 이름 검색" />
      <button>검색</button>
      <a class="btn gray" href="/admin">전체 보기</a>
      <a class="btn red" href="/admin?only_blocked=1">제외만 보기</a>
    </form>
  </div>
...
def admin_toggle(name: str = Form(...), action: str = Form(...)):
    blocked = load_blocked_monsters()
    name = str(name).strip()
...
@app.get("/download-blocked")
def download_blocked():
    if not os.path.exists(BLOCKED_JSON_PATH):
        save_blocked_monsters(set())

    return FileResponse(
        BLOCKED_JSON_PATH,
        media_type="application/json",
        filename="blocked_monsters.json",
    )
...
def match_monster(req: MatchRequest):
    try:
        ...
        results = run_clip_match(image_base64, top_k=3)
...
def save_result(req: SaveResultRequest):
    ...
    user_name = safe_user_name(req.user_name)
...
def result_page(result_id: str):
    ...
    cards = ""
    for idx, monster in enumerate(results[:3]):
        cards += f"""
        <div class="card {'top' if idx == 0 else ''}">
          <div class="rank">{idx + 1}위</div>
          <img src="{escape_html(monster.get('image_url', ''))}" />
          <h2>{escape_html(monster.get('name', '이름 없음'))}</h2>
          <b>매칭도 {monster.get('percent', 90)}%</b>
          <p>{escape_html(monster.get('reason', '전체 분위기가 비슷해요!'))}</p>
        </div>
        """
